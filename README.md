...
├── examples/            # query_rag.py（端到端）/ evaluate.py（评估 CLI）/ ingest_docs.py
├── tests/               # 180 个单元测试（含 tests/retriever/ Milvus mock 测试）
├── requirements.txt
├── Dockerfile
├── docker-compose.yml           # 主编排（rag-engine 服务）
├── docker-compose.milvus.yml    # 可选叠加：etcd + minio + Milvus
├── .env.example                 # 环境变量模板（LLM + Milvus）
└── .gitignore
```
...
| POST | `/ingest/file` | 上传 `.md` / `.pdf` / `.docx` 摄入（**上限 20MB**，超限返回 413） |
...
**错误码约定**：`400` 业务参数/召回失败（携带具体原因）· `413` 上传文件超限 · `500` 未预期异常（仅返回通用提示，细节进服务端日志）。
...
# 拉起 rag-engine(8000) + milvus(19530) + etcd + minio
```
...
MILVUS_PORT=19530
# 可选：MILVUS_USER / MILVUS_PASSWORD / MILVUS_DATABASE
```
...
**4. 索引参数**（`config/settings.yaml` 的 `retrieval.milvus` 段）：

| 参数 | 默认值 | 说明 |
|---|---|---|
| `collection` | `rag_chunks` | collection 名，首次使用自动创建 |
| `dim` | `384` | 向量维度，**必须与 embed 函数输出维度一致**（不一致会建索引失败） |
| `metric_type` | `COSINE` | 相似度度量：`COSINE` / `IP` / `L2`（L2 为距离，越小越相关） |
| `index_type` | `IVF_FLAT` | 索引类型（可换 `HNSW` / `IVF_SQ8` 等） |
| `nlist` / `nprobe` | `1024` / `16` | IVF 建索引桶数 / 查询探测桶数（调大 nprobe 提召回、增延迟） |

Milvus 召回器特性：
- 索引**持久化在服务端**，进程重启不丢失；支持增量 `upsert` 与按 `doc_id` 删除
- 使用真实 `embed_func`（工厂强制要求，缺失直接报错，避免误用假嵌入）；`dim` 与嵌入维度不一致时给出明确错误
- 单元测试全量 mock，无需真实服务（`tests/retriever/test_milvus_retriever.py`，L2/COSINE 过滤行为均有回归用例）

> 回退：把 `RAG_RETRIEVAL__RETRIEVER` 改回 `vector` / `hybrid` 并重启即可，Milvus 数据保留在服务端，不影响切换。
...
- `config/settings.yaml`：全局配置（各层可插拔选型 + LLM 密钥占位 + Milvus 连接/索引参数）
- 环境变量覆盖：`RAG_LLM__MODEL=xxx`（`RAG_` 前缀 + 双下划线分层）；Milvus 连接用 `MILVUS_*`（见 `.env.example`）
...
Python 3.13 · FastAPI · Pydantic · numpy · rank-bm25 · pypdf · python-docx · openai · PyYAML · pymilvus（可选）· Docker Compose
